...
type (
	...
	User struct {
		...
		Gender     int64     `db:"gender"`      // 性别(1男 2 女 3 保密)
...
func (m *defaultUserModel) Insert(data *User) (sql.Result, error) {
	...
	ret, err := m.Exec(func(conn sqlx.SqlConn) (result sql.Result, err error) {
		...
		return conn.Exec(query, data.Nickname, data.Mobile, data.Password, data.Gender)
...
func (m *defaultUserModel) Update(data *User) error {
	...
	_, err := m.Exec(func(conn sqlx.SqlConn) (result sql.Result, err error) {
		...
		return conn.Exec(query, data.Nickname, data.Mobile, data.Password, data.Gender, data.Id)
	}, userIdKey, userMobileKey)
	return err
}
